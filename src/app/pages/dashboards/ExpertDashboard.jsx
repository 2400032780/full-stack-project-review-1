...
import {
  FileText, MessageSquare, Users, TrendingUp, HelpCircle, CheckCircle, Clock, PlusCircle,
} from "lucide-react";
...
export function ExpertDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (response.ok) setQueries(await response.json());
    } catch (error) {
  ...
  const stats = [
    { icon: MessageSquare, title: "Total Questions", value: queries.length.toString(), change: "Across all farmers", color: "bg-primary/10 text-primary" },
    { icon: CheckCircle, title: "Solved Issues", value: queries.filter(q => q.status === "ANSWERED").length.toString(), change: "+2 today", color: "bg-green-100 text-green-600" },
    { icon: HelpCircle, title: "Pending Queries", value: queries.filter(q => q.status === "PENDING").length.toString(), change: "Need attention", color: "bg-amber-100 text-amber-600" },
    { icon: Users, title: "Active Experts", value: "12", change: "Online now", color: "bg-blue-100 text-blue-600" },
  ];
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Expert Control Center</h1>
        <p className="text-gray-600">Welcome back, {localStorage.getItem("userName") || "Expert"}. Help farmers with your insights.</p>
      </div>
        ...
        <TabsContent value="queries">
          <Card className="rounded-xl">
            <CardHeader>
              ...
              <CardDescription>Provide professional advice for the latest farming challenges</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  queries.map((query, index) => (
                    <div key={index} className="flex items-start justify-between p-4 border rounded-lg hover:bg-gray-50">
                      <div className="flex items-start gap-4 flex-1">
                        <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${query.status === "PENDING" ? "bg-amber-100" : "bg-green-100"}`}>
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <h4 className="font-semibold text-gray-900">
                              {query.farmerName || "Farmer #" + String(query.farmerId).slice(-4)}
                            </h4>
                            <Badge className="bg-blue-100 text-blue-800">{query.topic}</Badge>
                            <Badge className={getStatusBadgeColor(query.status)}>{query.status}</Badge>
                          </div>
